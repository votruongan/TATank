...
namespace Game.Base
{
	public class BaseClient
	{
		...
		public virtual bool Connect(Socket connectedSocket)
		{
			...
			if (this.m_sock.Connected)
			{
				int num = Interlocked.Exchange(ref this.m_isConnected, 1);
				if (num == 0)
				{
					this.OnConnect();
				}
		...
		{
			this.RECEIVE_KEY = StreamProcessor.cloneArray(StreamProcessor.KEY, 8);
			this.SEND_KEY = StreamProcessor.cloneArray(StreamProcessor.KEY, 8);
		}
